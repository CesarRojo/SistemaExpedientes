...
const ExamenMedico = () => {
  ...
  // useEffect(() => {
  //   const socket = io('http://172.30.189.106:5005'); // Asegúrate de que la URL sea correcta
  ...
  const fetchUsuarioFolio = async () => {
    try {
      ...
      console.log("fetchUsuarioFolio",response.data);
  ...
  const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            const response = await axios.post('http://172.30.189.106:5005/examMedico', {
                examMedicoData: {
