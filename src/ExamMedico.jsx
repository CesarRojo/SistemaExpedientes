...
const ExamenMedico = () => {
  ...
  // useEffect(() => {
  //   const socket = io('http://172.30.189.99:5005'); // Asegúrate de que la URL sea correcta
  ...
  const fetchAntecedentesPatologicos = async () => {
    try {
      const response = await axios.get('http://172.30.189.99:5005/antecPatolog');
  ...
  const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            const response = await axios.post('http://172.30.189.99:5005/examMedico', {
                examMedicoData: {
