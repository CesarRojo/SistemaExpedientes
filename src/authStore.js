...
const useAuthStore = create(persist(
  (set) => ({
    ...
    login: async (username, password) => {
      try {
        const response = await axios.post('http://172.30.189.95:5005/auth/login', { username, password });
    ...
    loginWithFolio: async (folio) => {
      try {
        const response = await axios.post('http://172.30.189.95:5005/auth/loginFolio', { folio });
    ...
    markVideosAsWatched: async (idFolio) => {
      try {
        await axios.put(`http://172.30.189.95:5005/folio/extras/${idFolio}`);
